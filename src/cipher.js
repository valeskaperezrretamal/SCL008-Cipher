//para eviar escribir dos vces el mismo codigo se definio una funcion que luego es usada dentro del objeto cipher.

window.cipher = {
  encode: (offset, mensaje)=> {
    return cifrar(offset,mensaje);
  }
   ,
...
function cifrar(offset,mensaje){
  let txtcifrado = "";
  let caracterAscii;
  let formula;
  let aux;
  ...
  for (let i= 0; i< mensaje.length; i++){
    caracterAscii = mensaje.charCodeAt(i);  //variable que guarda el carcater transformado en ascii
    
    /*se revisa que tipo de caracter que se va a codificar, puede ser mayuscula, minuscula, numero, 
    espacio u otros*/
    tipoDeCaracter = "otro"
    ...
    if (tipoDeCaracter === "otro") {
      txtcifrado = txtcifrado + mensaje[i];
    }
